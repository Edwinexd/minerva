...
- **Course knowledge graph**: documents are auto-classified (lecture, transcript, exercise, solution, ...) on `gpt-oss-120b` and cross-linked with `part_of_unit` / `solution_of` / `prerequisite_of` / `applied_in` edges. Retrieval expands top-k along the graph. Gated by the `course_kg` feature flag.
