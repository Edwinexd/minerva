//! Application error type, shared by the api crate (which renders it to
//! an HTTP response) and the axum-free worker / scheduler binaries (which
//! only ever construct / log it, e.g. the Canvas sync engine returning
//! `AppError` from `minerva_app_core::canvas::run_sync`).
//!
//! The enum, [`ErrorParams`], and [`LocalizedMessage`] are pure serde and
//! carry no framework dependency. The axum `IntoResponse` impl is gated
//! behind the crate's `axum` feature so building `minerva-worker` /
//! `minerva-scheduler` (which don't enable it) links no axum / tower.
//! Only the api crate (`minerva-server`) turns the feature on.
...
/// HTTP rendering of [`AppError`]. Lives behind the `axum` feature so only
/// the api crate links axum; the worker / scheduler binaries construct and
/// log `AppError` but never turn it into a `Response`.
#[cfg(feature = "axum")]
mod into_response {
    use super::{AppError, LocalizedMessage};
