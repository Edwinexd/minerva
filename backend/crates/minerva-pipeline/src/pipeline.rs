...
/// server-side dedup key across every upload entry point. Lives here next
/// to `extension_from_filename` so the api upload routes, the worker, and
/// the axum-free Canvas sync engine (in `minerva-app-core`) all share one
/// copy.
