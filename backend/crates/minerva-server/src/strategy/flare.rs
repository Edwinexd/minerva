...
pub async fn run(ctx: GenerationContext, tx: mpsc::Sender<Result<Event, AppError>>) {
    ...
    let loop_cfg = RunLoopConfig {
        ...
        history: &ctx.history,
        carryover: ctx.carryover.as_deref(),
...
struct RunLoopConfig<'a> {
    ...
    /// Mirror of `GenerationContext::carryover`; forwarded so the
    /// system prompt rebuilt on every loop iteration keeps the recap.
    carryover: Option<&'a str>,
    chat_base_url: &'a str,
    chat_api_key: &'a str,
    max_chunks: i32,
