          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left">
                ...
                <th className="py-2 pr-4 font-medium">{t("courses.columns.conceptGraph")}</th>
              ...
              {filtered.map((course) => {
                ...
                return (
                  <tr key={course.id} className="border-b">
                    ...
                    <td className="py-2 pr-4">
                      <ConceptGraphCell course={course} />
                    </td>
                    <td className="py-2">
                      <Link
...
// ── Concept knowledge graph (eureka-2) ────────────────────────────
//
// Per-course button that opens a dialog showing the persisted concept
// graph and exposing a "Run extraction" action. The button is disabled
// when the `concept_graph` feature flag is off for the course; toggling
// the flag is via `CourseFeatureFlagsCell` above.
//
// Extraction is bounded by however many documents the course has; the
// backend processes them sequentially. We let the request run to
// completion and surface a summary; aborting mid-run isn't supported
// (the work that's already happened is committed transactionally
// per-document, so an aborted run is recoverable by re-running).
...
function ConceptGraphCell({ course }: { course: Course }) {
  const { t } = useTranslation("admin")
  const formatError = useApiErrorMessage()
  const queryClient = useQueryClient()
  const [open, setOpen] = useState(false)

  const enabled = course.feature_flags?.concept_graph === true

  const graphQuery = useQuery({
    queryKey: ["admin", "courses", course.id, "concept-graph"],
    queryFn: () =>
      api.get<ConceptGraphResponse>(
        `/admin/courses/${course.id}/concept-graph`,
      ),
  ...
  const extractMutation = useMutation({
    mutationFn: () =>
      api.post<ExtractRunSummary>(
        `/admin/courses/${course.id}/concept-graph/extract`,
        {},
      ),
    onSuccess: () => {
      // Refresh the graph so the UI shows the freshly-persisted concepts.
  ...
  if (!enabled) {
    return (
      <Button
        ...
        size="sm"
        disabled
        title={t("courses.conceptGraphDisabledTitle")}
      >
        {t("courses.conceptGraphButtonDisabled")}
      </Button>
    )
  }

  const concepts = graphQuery.data?.graph.concepts ?? []
  const edges = graphQuery.data?.graph.edges ?? []
  const summary = extractMutation.data

  return (
    <>
      <Button variant="outline" size="sm" onClick={() => setOpen(true)}>
        {t("courses.conceptGraphButton")}
      </Button>
      <AlertDialog open={open} onOpenChange={setOpen}>
        <AlertDialogContent className="max-w-3xl">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("courses.conceptGraphDialogTitle", { course: course.name })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("courses.conceptGraphDialogDescription")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              >
                {extractMutation.isPending
                  ? t("courses.conceptGraphExtractRunning")
                  : t("courses.conceptGraphExtractRun")}
              </Button>
              <span className="text-xs text-muted-foreground">
                {t("courses.conceptGraphCounts", {
            ...
            {extractMutation.error ? (
              <p className="text-sm text-destructive">
                {formatError(extractMutation.error)}
              </p>
            ) : null}

            {summary ? (
              <div className="rounded border bg-muted/30 p-2 text-xs">
                {t("courses.conceptGraphRunSummary", {
                  processed: summary.documents_processed,
                  skipped: summary.documents_skipped,
                  concepts: summary.concepts_added,
                  edges: summary.edges_added,
            ...
            <div className="max-h-[50vh] overflow-y-auto rounded border">
              {graphQuery.isLoading ? (
                ...
                <p className="p-3 text-sm text-destructive">
                  {formatError(graphQuery.error)}
                </p>
              ) : concepts.length === 0 ? (
                <p className="p-3 text-sm text-muted-foreground">
                  {t("courses.conceptGraphEmpty")}
                </p>
              ) : (
