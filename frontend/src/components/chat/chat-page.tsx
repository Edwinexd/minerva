...
// Exported so it can be reused wherever the full chat UX
// (transcript + composer + Aegis panel) is needed without the
// conversation-list sidebar that ChatPage wraps it with.
      ...
      try {
        const res = await fetch(`/api/courses/${courseId}/aegis/rewrite`, {
          ...
          body: JSON.stringify({
            ...
            mode,
            // Threaded so the backend's per-conversation Aegis gate
            // can scope rewrites to the right conversation. Null is
            // fine for brand-new composers (no conv yet); the backend
            // falls back to the umbrella in that case.
            conversation_id: conversationId,
          }),
        })
        if (!res.ok) {
